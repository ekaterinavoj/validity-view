    ...
    if (tlsMode === "starttls") {
      ...
      if (resp.code === 220) {
        try {
          ...
          // Fall through to plain SMTP below
          connection = await Deno.connect({ hostname: host, port });
          
          const pr = connection.readable.getReader();
          const pw = connection.writable.getWriter();
          
          const readP = async (): Promise<string> => {
            const d = new TextDecoder(); let r = "";
            while (true) { const { value, done } = await pr.read(); if (done) break; r += d.decode(value, { stream: true }); if (r.includes("\r\n")) { const l = r.split("\r\n"); const ll = l[l.length-2]||l[l.length-1]; if (ll.length>=4 && ll[3]!=='-') break; } } return r;
          };
          ...
          const emailData = [`From: "${fromName}" <${fromEmail}>`,`To: ${toRecipients.join(", ")}`,bccRecipients.length>0?`Bcc: ${bccRecipients.join(", ")}`:"",`Subject: =?UTF-8?B?${btoa(unescape(encodeURIComponent(subject)))}?=`,"MIME-Version: 1.0",`Content-Type: text/html; charset=UTF-8`,"",body,"."].filter(Boolean).join("\r\n");
          await pw.write(new TextEncoder().encode(emailData + "\r\n"));
          await sendP("QUIT");
