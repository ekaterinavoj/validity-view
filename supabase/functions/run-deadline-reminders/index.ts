...
const handler = async (req: Request): Promise<Response> => {
  ...
  if (frequency.dual_mode) {
    ...
    if (shouldSendExpired && expiredItems.length > 0) {
      ...
      const bodyTextRaw = replaceVariables(bodyTemplate, expiredItems.length, 0, expiredItems.length, expiredItems);
      const tableHtml = buildDeadlinesTable(expiredItems, 'expired');
      const hasRecordsTableVar = /\{\{records_table\}\}/.test(bodyTextRaw);
      const bodyText = bodyTextRaw.replace(/\{\{records_table\}\}/g, tableHtml);
      const fullBody = `
        <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; color: #333;">
          <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            ${bodyText.replace(/\n/g, "<br>")}
          </div>
          ${hasRecordsTableVar ? '' : tableHtml}
    ...
    if (shouldSendWarning && warningItems.length > 0) {
      ...
      const bodyTextRaw = replaceVariables(bodyTemplate, warningItems.length, warningItems.length, 0, warningItems);
    ...
    const bodyTextRaw = replaceVariables(bodyTemplate, totalCount, expiringCount, expiredCount, allDeadlineItems);
