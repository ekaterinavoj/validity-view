            ...
            <TableBody>
              {trainingTypes.length === 0 ? (
                ...
                trainingTypes.map((type) => {
                  const years = (type.period_days / 365).toFixed(1);
                  const months = (type.period_days / 30).toFixed(1);
                  const displayPeriod = 
                    type.period_days % 365 === 0 ? `${years} ${parseFloat(years) === 1 ? 'rok' : parseFloat(years) < 5 ? 'roky' : 'let'}` :
                    type.period_days % 30 === 0 ? `${months} ${parseFloat(months) === 1 ? 'měsíc' : parseFloat(months) < 5 ? 'měsíce' : 'měsíců'}` :
                    `${type.period_days} ${type.period_days === 1 ? 'den' : type.period_days < 5 ? 'dny' : 'dní'}`;
