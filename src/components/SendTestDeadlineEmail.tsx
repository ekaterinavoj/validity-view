...
interface SendTestDeadlineEmailProps {
  hasRecipients: boolean;
  isEnabled: boolean;
}

export function SendTestDeadlineEmail({ isEnabled }: SendTestDeadlineEmailProps) {
  ...
  const handleSendTest = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("run-deadline-reminders", {
        body: {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => open ? setIsOpen(true) : handleClose()}>
      <DialogTrigger asChild>
        ...
        >
          ...
          Odeslat náhled na jednu adresu (Technické lhůty)
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Spustí skutečnou kontrolu technických lhůt a pošle vygenerovaný souhrnný email
          ...
          {result ? (
            ...
            <>
              <div className="space-y-2">
                <Label htmlFor="test-email-deadline">Testovací emailová adresa</Label>
                <Input
                  id="test-email-deadline"
                  type="email"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Email bude obsahovat aktuální data technických lhůt a bude označen jako [TEST].
